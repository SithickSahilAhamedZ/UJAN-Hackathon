...
const App = () => {
  const [auth, setAuth] = useState<{ role: 'user' | 'admin' | null, token: string | null }>({ role: null, token: null });
  ...
  useEffect(() => {
    const storedAuth = localStorage.getItem('pilgrimPathAuth');
    if (storedAuth) {
  ...
  const handleLogin = (role: 'user' | 'admin', token: string) => {
    ...
    setAuth(authData);
    localStorage.setItem('pilgrimPathAuth', JSON.stringify(authData));
    setActivePage(role === 'admin' ? Page.Analytics : Page.Home);
  };
  
  const handleLogout = () => {
    setAuth({ role: null, token: null });
    localStorage.removeItem('pilgrimPathAuth');
